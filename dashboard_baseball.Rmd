...
renderDataTable({
  datatable(
  subset(stats, nom_joueur %in% input$Joueur),
  filter ="top",
...
renderPlotly({
  stats_graph <- subset(stats, nom_joueur %in% input$Joueur)
...
##test
>>>>>>> 306d4a5c5baa37f95cbc3e8d8e7d1884dd68f9c6
...
# Regrouper par équipe et calculer les moyennes
...
stats_equipes <- stats_1 %>%
  ...
  summarise(
    ...
    double_jeu_total = sum(double_jeu, na.rm = TRUE),
    ...
    pourcent_sweet_spot_moyen = mean(pourcent_sweet_spot, na.rm = TRUE),
    vitesse_moy_frappe_moyenne = mean(vitesse_moy_frappe, na.rm = TRUE),
    pourcent_elans_manques_moyen = mean(pourcent_elans_manques, na.rm = TRUE),
    pourcent_elans_total_moyen = mean(pourcent_elans_total, na.rm = TRUE)
  ) %>%
  arrange(desc(OPS_moyen))
...
# Menu pour choisir la statistique

selectInput("stat_sel", "Choisissez une statistique :",
            choices = c(
              "OPS moyen" = "OPS_moyen",
              "Taux de frappe puissante" = "taux_frappe_puissante_moyen",
              "Pourcentage de frappe forte" = "pourcent_frappe_forte_moyen",
              "Vitesse moyenne de frappe" = "vitesse_moy_frappe_moyenne"
            ),
            selected = "OPS_moyen")
...
renderPlot({
  stat_var <- input$stat_sel
  ggplot(stats_equipes, aes_string(x = "reorder(equipe, desc(" %+% stat_var %+% "))", y = stat_var)) +
    geom_col(fill = "steelblue") +
    coord_flip() +
    labs(title = paste("Statistique :", stat_var), x = "", y = "") +
    theme_minimal()
})
