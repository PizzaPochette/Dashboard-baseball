...
recherche<- fluidPage(
  fluidRow(
    column(4,
      selectizeInput(
        "Joueur", "Sélectionne un ou plusieurs joueurs :",
        choices =sort( unique(stats$nom_joueur)),
        multiple = TRUE
      )
    ),
    column(4,
      selectInput(
        "xvar", "Choisir l’axe X :", 
        choices = names(stats), 
        selected = "retraits_au_sol"
      )
    ),
    column(4,
      selectInput(
        "yvar", "Choisir l’axe Y :", 
        choices = names(stats), 
        selected = "retraits_ligne"
      )
    )
  ),
...
fluidRow(
  column(12, plotlyOutput("graph"))
)
)

dehors <- function(input, output) {
output$table <- renderDataTable({
  datatable(
  subset(stats, nom_joueur %in% input$Joueur),
  filter ="top",
  options = list(scrollX=TRUE, autoWidth = TRUE, dom='Brtip', buttons = c('colvis'), 
    columnDefs = list(list(visible = TRUE, targets = 0:6),list(visible = FALSE, targets = 7:20)) ),
  extensions = "Buttons"
...
output$graph <- renderPlotly({
  ...
  req(input$Joueur, input$xvar, input$yvar)
  ggplotly(
    ggplot(stats_graph, aes_string(
      x= input$xvar,
      y= input$yvar,
      color= "nom_joueur")
            )
